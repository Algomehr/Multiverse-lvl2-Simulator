...
export const ControlPanel: React.FC<ControlPanelProps> = ({ constants, setConstants, mode, setMode, onSimulate, isLoading }) => {
  ...
  const handleGoalDesign = async () => {
    ...
    setError('');
    try {
      const newConstantValues = await getConstantsFromGoal(goal);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6 bg-slate-900/70 backdrop-blur-sm border border-slate-700 rounded-2xl shadow-2xl shadow-indigo-900/20">
      <div className="mb-6 flex flex-col sm:flex-row justify-center items-center gap-4">
        ...
        >
          <BeakerIcon /> Researcher Mode
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        {Object.values(constants).map(c => (
          <PhysicsSlider key={c.id} constant={c} onChange={handleSliderChange} />
        ))}
      </div>

      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-center text-indigo-300">Or... Design a Universe with a Goal</h3>
        <textarea
          value={goal}
          onChange={(e) => setGoal(e.target.value)}
          placeholder="e.g., 'A universe with very large, short-lived stars' or 'جهانی با ستاره‌های بسیار بزرگ و عمر کوتاه'"
        ...
        <div className="flex justify-center gap-4">
            ...
            >
            {isDesigning ? 'Designing...' : 'Design Constants'}
            </button>
            <button
                onClick={onSimulate}
                disabled={isLoading}
            ...
            >
                {isLoading ? "Simulating..." : "CREATE UNIVERSE"}
            </button>
        </div>
      </div>
    </div>
